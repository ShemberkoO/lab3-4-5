...
public class PersonController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<PersonDto>>> GetPeople()
    {
        var peopleDto = await _personRepository.GetPeopleAsync();
        return Ok(peopleDto);
    }
    ...
    public async Task<ActionResult<PersonDto>> PostPerson(PersonDto personDto)
    {
        await _personRepository.AddPersonAsync(personDto);
        return CreatedAtAction(nameof(GetPerson), new { id = personDto.PasportId }, personDto);
    }
